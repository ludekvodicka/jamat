...
describe('lib-orchestrator/shared/tortoiseCommitDialog', () => {
  ...
  it.each(['svn', 'git'] as const)('opens %s with literal paths, a message file and the human Git environment', async (vcs) => {
    ...
    expect(launchInteractive).toHaveBeenCalledExactlyOnceWith({
      ...
      cwd: process.cwd(), args: ['/command:commit', '/path:Q:/app with spaces', '/logmsgfile:Q:/temp/message.txt'], env: expect.any(Object),
    })
  ...
  it('preserves an explicit file selection when the person opens Tortoise', async () => {
    const launchInteractive = vi.fn<CommandInvoker['launchInteractive']>(async () => ({ ok: true, closed: Promise.resolve() }))
    const dialog = new TortoiseCommitDialog({ platform: 'win32', exists: () => true, commands: { launchInteractive } })
    await dialog.open({ vcs: 'svn', scope: 'Q:/app', paths: ['Q:/app/a @.txt', 'Q:/app/b.txt'], messageFile: null })
    expect(launchInteractive.mock.calls[0][0]).toMatchObject({ cwd: process.cwd(), args: ['/command:commit', '/path:Q:/app/a @.txt*Q:/app/b.txt'] })
  })
})
