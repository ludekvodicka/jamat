...
import type { StatsView, ProjectSummary, ModelSummary24h, ProjectModelCell } from '../../../../../../core/types/stats'
...
const proj = (project: string, cost: number): ProjectSummary => ({
  ...
  reasoningTokens: 0, cost, durationMs: 1000, requestCount: 5, sessionCount: 1, modelsUsed: ['claude-opus-4-8'],
})
const model = (m: string, cost: number): ModelSummary24h => ({
  ...
  reasoningTokens: 0, cost, durationMs: 1000, requestCount: 5, sessionCount: 1,
})
const cell = (cost: number): ProjectModelCell => ({
  ...
  reasoningTokens: 0, cost, durationMs: 500, requestCount: 2, sessionCount: 1,
})
...
const STATS: StatsView = {
  ...
  hourly24h: [{
    ...
    reasoningTokens: 0, cost: 0.5, durationMs: 1000, modelsUsed: ['claude-opus-4-8'], projects: ['A'],
    byProject: { A: { inputTokens: 100, outputTokens: 50, cacheCreationTokens: 0, cacheReadTokens: 0, reasoningTokens: 0, cost: 0.5, durationMs: 1000 } },
    byModel: { 'claude-opus-4-8': { inputTokens: 100, outputTokens: 50, cacheCreationTokens: 0, cacheReadTokens: 0, reasoningTokens: 0, cost: 0.5, durationMs: 1000 } },
  }],
...
describe('Last24hTab', () => {
  ...
  it('hides API-time cards and columns when duration is unavailable', () => {
    const { container } = render(<Last24hTab stats={{ ...STATS, durationCoverage: 'none' }} />)
