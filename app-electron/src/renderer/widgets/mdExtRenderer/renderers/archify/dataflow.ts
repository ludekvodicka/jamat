/* Vendored from archify — github.com/tt-a1i/archify, MIT (renderers/dataflow/render-dataflow.mjs).
 * Ported to a pure `renderDataflow(diagram) → svg` (no fs / HTML template / theme JS / cards). See ./LICENSE. */
...
export function renderDataflow(dataflow: any): string {
  ...
  function measureNode(node: any) {
    ...
    return { ...node, width, height, cx, cy: y + height / 2, x: cx - width / 2, y }
  }

  const nodes = new Map<string, any>(asArray(dataflow.nodes).map((node: any) => [node.id, measureNode(node)]))

  function routeVia(flow: any, from: any, to: any, start: Point, end: Point): Point[] {
  ...
  function pathFor(flow: any) {
    ...
    const points: Point[] = [start, ...routeVia(flow, from, to, start, end), end]
  ...
  function validateDataflow() {
    ...
    if (!Array.isArray(dataflow.stages) || dataflow.stages.length < 2) problems.push('Data-flow diagrams need at least two stages.')
    if (!Array.isArray(dataflow.nodes) || dataflow.nodes.length < 2) problems.push('Data-flow diagrams need at least two nodes.')
    ...
    if (problems.length) throw new Error(`Data-flow layout validation failed:\n- ${problems.join('\n- ')}`)
  }

  function renderStage(stage: any, index: number) {
  ...
  function renderFlowPath(flow: any) {
  ...
  function renderFlowLabel(flow: any) {
    const routed = pathFor(flow)
