/* Vendored from archify — github.com/tt-a1i/archify, MIT (renderers/lifecycle/render-lifecycle.mjs).
 * Ported to a pure `renderLifecycle(diagram) → svg` (no fs / HTML template / theme JS / cards). See ./LICENSE. */
...
export function renderLifecycle(lifecycle: any): string {
  ...
  function bandFor(lane: string) {
    if (lane === 'main') return 'phase'
    if (lane === 'terminal') return 'outcome'
    return 'event'
  }

  function measureState(state: any) {
    ...
    return { ...state, width, height, x: cx - width / 2, y, cx, cy: y + height / 2 }
  }

  const states = new Map<string, any>(asArray(lifecycle.states).map((state: any) => [state.id, measureState(state)]))

  function routeVia(transition: any, from: any, to: any, start: Point, end: Point): Point[] {
  ...
  function pathFor(transition: any) {
    ...
    const points: Point[] = [start, ...routeVia(transition, from, to, start, end), end]
    const routed = { d: roundedPath(points, transition.cornerRadius ?? 10), points }
  ...
  function validateLifecycle() {
    ...
    const laneIds = new Set(asArray(lifecycle.lanes).map((lane: any) => lane.id))
    ...
    for (const state of states.values()) {
      ...
      const maxCol = band === 'phase' ? layout.phaseXs.length : band === 'outcome' ? layout.outcomeXs.length : layout.eventXs.length
    ...
    if (problems.length) throw new Error(`Lifecycle layout validation failed:\n- ${problems.join('\n- ')}`)
  }

  function bandTitles() {
    const lanes = asArray(lifecycle.lanes)
    const mainLane = lanes.find((lane: any) => lane.id === 'main')
    const terminalLane = lanes.find((lane: any) => lane.id === 'terminal')
    const eventLanes = lanes.filter((lane: any) => lane.id !== 'main' && lane.id !== 'terminal')
    return [
      mainLane?.label || 'Lifecycle phases',
      eventLanes.length ? eventLanes.map((lane: any) => lane.label).join(' + ') : 'Interruptions + recovery',
  ...
  function renderTransitionPath(transition: any) {
  ...
  function renderTransitionLabel(transition: any) {
  ...
  function renderLifecycleRail() {
    const mainCols = [...states.values()].filter((state: any) => bandFor(state.lane) === 'phase').map((state: any) => state.col)
