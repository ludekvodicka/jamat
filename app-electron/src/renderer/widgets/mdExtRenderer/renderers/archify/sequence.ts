/* Vendored from archify — github.com/tt-a1i/archify, MIT (renderers/sequence/render-sequence.mjs).
 * Ported to a pure `renderSequence(diagram) → svg` (no fs / HTML template / theme JS / cards). See ./LICENSE. */
/* eslint-disable @typescript-eslint/no-explicit-any */
import { esc, renderDefinitions, textUnits } from './utils'
import { componentFill, arrowClassMap, rectsOverlap, asArray } from './geometry'

export function renderSequence(sequence: any): string {
  ...
  const arrowClass: Record<string, [string, string]> = {
    ...arrowClassMap,
  ...
  const participants = new Map<string, any>(
    asArray(sequence.participants).map((participant: any, index: number) => [
      participant.id,
      { ...participant, index, cx: participantX(index), x: participantX(index) - layout.participantW / 2 },
    ]),
  )
  ...
  function validateSequence() {
    ...
    const placed = asArray(sequence.messages)
      .filter((m: any) => participants.has(m.from) && participants.has(m.to))
      .map((m: any) => ({
        label: m.label,
        y: m.y,
    ...
    const labelRects = asArray(sequence.messages)
      .filter((m: any) => participants.has(m.from) && participants.has(m.to) && typeof m.y === 'number')
      .map((m: any) => {
    ...
    if (problems.length) throw new Error(`Sequence layout validation failed:\n- ${problems.join('\n- ')}`)
  }

  function renderParticipant(participant: any) {
  ...
  function renderLifeline(participant: any) {
  ...
  function renderSegment(segment: any) {
  ...
  function renderActivation(activation: any) {
  ...
  function messageLabel(message: any, x1: number, x2: number) {
