/* Vendored from archify — github.com/tt-a1i/archify, MIT (renderers/workflow/render-workflow.mjs).
 * Ported to a pure `renderWorkflow(diagram) → svg` (no fs / HTML template / theme JS / cards). See ./LICENSE. */
/* eslint-disable @typescript-eslint/no-explicit-any */
import { esc, renderDefinitions, textUnits } from './utils'
import {
  asArray,
  isFinitePoint,
  rectsOverlap,
...
export function renderWorkflow(workflow: any): string {
  const layout = {
    laneX: 40,
    laneY: 52,
    laneW: 640,
  ...
  const laneIndex = new Map<string, number>(asArray(workflow.lanes).map((lane: any, index: number) => [lane.id, index]))

  function laneTop(id: string) {
    return layout.laneY + (laneIndex.get(id) as number) * (layout.laneH + layout.laneGap)
  }
  ...
  function measureNode(node: any) {
    ...
    return { ...node, width, height, x: cx - width / 2, y, cx, cy: y + height / 2 }
  }

  const nodes = new Map<string, any>(asArray(workflow.nodes).map((node: any) => [node.id, measureNode(node)]))

  function gapYBetween(fromLane: string, toLane: string, bias = 0.5) {
    const a = laneTop(fromLane) + layout.laneH
    const b = laneTop(toLane)
    return a + (b - a) * bias
  }

  function routeVia(edge: any, from: any, to: any, start: Point, end: Point): Point[] {
  ...
  const pathCache = new Map<any, { d: string; points: Point[] }>()
  function pathFor(edge: any) {
    ...
    const points: Point[] = [start, ...routeVia(edge, from, to, start, end), end]
  ...
  function validateWorkflow() {
    ...
    if (workflow.schema_version !== 1) problems.push('Workflow files must set "schema_version": 1.')
    if (workflow.diagram_type !== 'workflow') problems.push(`Unsupported diagram_type "${workflow.diagram_type}". Expected "workflow".`)
    if (!workflow.meta || !workflow.meta.title) problems.push('Workflow files must include meta.title.')
    if (!Array.isArray(workflow.lanes) || !workflow.lanes.length) problems.push('Workflow files must include at least one lane.')
    if (!Array.isArray(workflow.nodes)) problems.push('Workflow files must include a nodes array.')
    if (!Array.isArray(workflow.edges)) problems.push('Workflow files must include an edges array.')
    if (workflow.cards !== undefined && !Array.isArray(workflow.cards)) problems.push('Workflow "cards" must be an array.')
    if (problems.length) throw new Error(`Workflow layout validation failed:\n- ${problems.join('\n- ')}`)

    const laneIds = new Set(workflow.lanes.map((lane: any) => lane.id))
    if (laneIds.size !== workflow.lanes.length) problems.push('Lane ids must be unique.')
    if (nodes.size !== workflow.nodes.length) problems.push('Node ids must be unique.')
  ...
  function renderLane(lane: any, index: number) {
  ...
  function renderEdgePath(edge: any) {
  ...
  function renderEdgeLabel(edge: any) {
  ...
  function svgRootAttrs(meta: any, kind: string) {
