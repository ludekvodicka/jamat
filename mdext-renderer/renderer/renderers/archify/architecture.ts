/* Vendored from archify — github.com/tt-a1i/archify, MIT (renderers/architecture/render-architecture.mjs).
 * Ported to a pure browser function: takes the parsed `architecture` diagram object and RETURNS the
 * `<svg>` string (no fs, no HTML template, no theme-toggle JS, no export menu, no info cards). The
 * layout math, validation, routing, and SVG markup are archify's, unchanged. Styling is via CSS
 * classes (see `renderer/mdExtRenderer.css`; upstream's archify.css is not used - see VENDOR.md). See ./LICENSE. */
...
/** Render an archify `architecture` diagram object to an `<svg>` string. Throws on layout problems
 *  (the message names the fix) — the Diagram leaf turns that into an in-pane error + show-source. */
export function renderArchitecture(arch: any): string {
  const layout = {
    defaultW: 120,
    defaultH: 60,
    margin: 40,
  ...
  function measureComponent(c: any) {
  ...
  const components = new Map<string, any>(asArray(arch.components).map((c: any) => [c.id, measureComponent(c)]))

  // ---- Boundaries computed from the `wraps` id list -------------------------
  function boundaryRect(boundary: any) {
    const members = asArray(boundary.wraps)
      .map((id: string) => components.get(id))
      .filter(Boolean)
    if (!members.length) return null
    const minX = Math.min(...members.map((m: any) => m.x))
    const minY = Math.min(...members.map((m: any) => m.y))
    const maxX = Math.max(...members.map((m: any) => m.x + m.width))
    const maxY = Math.max(...members.map((m: any) => m.y + m.height))
  ...
  const boundaries = asArray(arch.boundaries).map(boundaryRect).filter(Boolean) as any[]
  ...
  // ---- Connection routing --------------------------------------------------
  function routeVia(conn: any, _from: any, _to: any, start: Point, end: Point): Point[] {
  ...
  function pathFor(conn: any) {
    ...
    const points: Point[] = [start, ...routeVia(conn, from, to, start, end), end]
  ...
  function renderConnectionPath(conn: any) {
  ...
  function renderConnectionLabel(conn: any) {
