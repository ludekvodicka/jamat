...
| Links / images | Relative links and images render through FileViewer capabilities; HTTP(S) links open only after a click. Unsafe schemes are dropped. |
...
- **External images never auto-load.** Images must be relative resources allowed by FileViewer.
...
- Archify specs are valid JSON, set `diagram_type`, place nodes without overlaps / off-canvas, and
  ...
  they don't overlap a node. A failed layout renders its source instead of a broken diagram.
