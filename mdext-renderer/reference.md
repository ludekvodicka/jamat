...
Companion to [USAGE.md](./USAGE.md). Details for authoring `.md` / `.mdext` that the `MdExtRenderer`
widget renders.
...
MUI-free; themed entirely through CSS variables (`--mdext-fg`, `--mdext-code-bg`, ...). AppJamatV3
sets them on the FileViewer scope. Diagrams recolor when the application theme changes.

## Diagram engines — overview

Engines live in `renderer/renderers/mdExtDiagramEngine.ts`; each fence language routes to one
...
See the decision table in [USAGE.md](./USAGE.md#diagrams--charts--pick-the-right-engine). In short:
...
> clear space with `labelAt: [x, y]`. A layout error falls back to the readable source in the viewer.
...
- **Auto-loading external images** - FileViewer resolves only relative resources within its grant.

## Host integration (for app developers)

The maintained renderer and authoring contract live together in `AppJamatV3/mdext-renderer/`.
FileViewer imports the public exports from `renderer/index.ts` and supplies capability-bound image
and link handlers:

`<MdExtRenderer source={text} resolveImage={resolveImage} onLink={onLink} className? />`.

Thin discovery adapters under `AppJamatV3/skills/claude` and `AppJamatV3/skills/codex` point to this
directory. `app-client-ui/app/skills/skillLinkInstaller.ts` owns both profile links. Update this
guidance in the same change whenever the renderer's authorable format changes.
