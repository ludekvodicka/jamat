import { Resvg } from '@resvg/resvg-js'
import { nativeImage, type NativeImage } from 'electron'

export class WindowIcon {
  private static readonly defaultLogoPngBase64Const = ''
  private static readonly luminanceThresholdConst = 0.36
  private static readonly colorPatternConst = /^#[0-9a-f]{6}$/
  private static readonly cache = new Map<string, NativeImage>()

  static of(color: string | null): NativeImage {
    const key = color ?? ''
    const cached = WindowIcon.cache.get(key)
    if (cached)
      return cached
    if (key !== '' && !WindowIcon.colorPatternConst.test(key))
      throw new Error(`Invalid icon color: ${JSON.stringify(color)}`)
    const icon = key === ''
      ? nativeImage.createFromDataURL(
        `data:image/png;base64,${WindowIcon.defaultLogoPngBase64Const}`,
      )
      : nativeImage.createFromBuffer(WindowIcon.renderTinted(key))
    if (icon.isEmpty())
      throw new Error(`Window icon rendering produced an empty image for ${JSON.stringify(color)}`)
    WindowIcon.cache.set(key, icon)
    return icon
  }

  /**
   * For the tests, and only for them. The cache is static and lives as long as the module, so a
   * second test asking for the same colour would be answered by the first one's image and would
   * prove nothing about rendering. There is nothing in the product that wants a cold cache.
   */
  static clearCache(): void {
    WindowIcon.cache.clear()
  }

  private static renderTinted(color: string): Buffer {
    const lightBackground = WindowIcon.relativeLuminance(color)
      > WindowIcon.luminanceThresholdConst
    const ink = lightBackground ? '#0d1117' : '#ffffff'
    const border = lightBackground ? 'rgba(0,0,0,0.30)' : 'rgba(255,255,255,0.16)'
    const svg =
      '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 120 120" width="120" height="120">'
      + `<rect x="1" y="1" width="118" height="118" rx="26" fill="${color}" stroke="${border}" stroke-width="2"/>`
      + `<path d="M 38 36 H 82" stroke="${ink}" stroke-width="12" stroke-linecap="round" fill="none"/>`
      + `<path d="M 70 36 V 72 Q 70 92 51 92 Q 37 92 35 78" stroke="${ink}" stroke-width="12" stroke-linecap="round" fill="none"/>`
      + `<rect x="80" y="82" width="16" height="12" rx="2" fill="${ink}"/>`
      + '</svg>'
    return Buffer.from(
      new Resvg(svg, { fitTo: { mode: 'width', value: 256 } }).render().asPng(),
    )
  }

  private static relativeLuminance(color: string): number {
    const red = WindowIcon.channel(color.slice(1, 3))
    const green = WindowIcon.channel(color.slice(3, 5))
    const blue = WindowIcon.channel(color.slice(5, 7))
    return 0.2126 * red + 0.7152 * green + 0.0722 * blue
  }

  private static channel(hex: string): number {
    const channel = Number.parseInt(hex, 16) / 255
    return channel <= 0.03928
      ? channel / 12.92
      : Math.pow((channel + 0.055) / 1.055, 2.4)
  }
}

